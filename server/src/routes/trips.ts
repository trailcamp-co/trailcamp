...
router.get('/:id/export-markdown', (req: Request, res: Response) => {
  ...
  lines.push('');
  lines.push(`**Status:** ${trip.status} | **Start:** ${trip.start_date || 'TBD'}`);

  const totalNights = stops.reduce((s: number, st: any) => s + (st.nights || 0), 0);
  const totalMiles = stops.reduce((s: number, st: any) => s + (st.drive_distance_miles || 0), 0);
  ...
  stops.forEach((stop, i) => {
    ...
    if (stop.drive_distance_miles) lines.push(`🚗 ${Math.round(stop.drive_distance_miles)} mi${stop.drive_time_minutes ? ` (~${Math.round(stop.drive_time_minutes / 60)}h ${Math.round(stop.drive_time_minutes % 60)}m)` : ''}`);
  ...
  res.setHeader('Content-Disposition', `attachment; filename="${trip.name.replace(/[^a-zA-Z0-9 -]/g, '')}.md"`);
  res.send(lines.join('\n'));
});
